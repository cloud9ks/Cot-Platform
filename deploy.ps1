...
try {
    ...
    Write-Host "  ✓ Docker Compose installato: $composeVersion" -ForegroundColor Green
} catch {
...
if (-Not (Test-Path ".env")) {
    ...
    Write-Host "  ⚠️  ATTENZIONE: Modifica il file .env con i tuoi valori!" -ForegroundColor Red
...
foreach ($dir in $dirs) {
    if (-Not (Test-Path $dir)) {
        ...
        Write-Host "  ✓ Esiste: $dir" -ForegroundColor Green
    }
}
...
docker-compose up -d
if ($LASTEXITCODE -ne 0) {
    Write-Host "  ✗ Avvio fallito!" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "  ⚠️  Applicazione non ancora pronta (riprova tra 30 secondi)" -ForegroundColor Yellow
}
...
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "🎉 La tua piattaforma COT è online!" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "📖 Documentazione completa: DEPLOY_WINDOWS_SERVER.md" -ForegroundColor Gray
Write-Host ""
